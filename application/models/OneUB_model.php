...
class One_model extends CI_Model
{
    ...
    public function __construct()
    {
        ...
        // $this->load->database($this->session->database, FALSE, TRUE);
        // }
    }
    ...
    // 28/10/22 update: will be deprecated. all list will use get_list fx
    ...
    public function get_ref($table, $dropdown = false, $options = array())
    {
        ...
        if (isset($options['add_new'])) {
            $array['add_new'] = $options['add_new'];
        }
    ...
    public function insert($table, $data)
    {
        ...
        if ($this->db->field_exists('uuid', $table)) {
            $hash_id = hash_encode($id, array('length' => '17'));
            $this->update($table, array('uuid' => $hash_id), $id);
        }
        return $id;
    }
    ...
    public function delete($table, $id, $primaryfield = "id", $soft_delete = TRUE)
    {
        if ($soft_delete) {
